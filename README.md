...
3. Run the exporter:
   ...
   ```
   For diagnostics without publishing over MQTT, use `--dry-run` and `--log-level DEBUG`.
...
LibreHardwareMonitor must expose its JSON endpoint (default is `http://localhost:8085/data.json`). On Linux, `lsblk` and `smartctl` are used when present to enrich drive metadata and SMART stats.
